    ...
    <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-stone-200 rounded-2xl p-5">
          <div className="text-[11px] uppercase tracking-wider text-stone-500 font-semibold mb-1">Boîtes en stock</div>
          <div className="text-3xl font-bold text-stone-900 tabular-nums">{totalBoxes}</div>
        </div>
        <div className="bg-white border border-stone-200 rounded-2xl p-5">
          <div className="text-[11px] uppercase tracking-wider text-stone-500 font-semibold mb-1">Valeur stock</div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-stone-50 text-[11px] uppercase tracking-wider text-stone-500">
            <tr>
              ...
              <th className="text-right px-4 py-3 hidden md:table-cell">Unités</th>
              <th className="text-right px-4 py-3 hidden md:table-cell">Coût/unité</th>
              ...
              <tr key={r.product_id} className={`border-t border-stone-200 ${r.low_stock ? 'bg-amber-50/50' : ''}`}>
                ...
                <td className="px-4 py-3 text-center">
                  {r.low_stock
                    ...
                    : <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700">OK</span>
                  }
                </td>
              </tr>
            ))}
            {!isLoading && rows.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-stone-400">Aucun produit.</td></tr>
            )}
          </tbody>
        </table>
